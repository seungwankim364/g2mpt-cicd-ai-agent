...
case "$ACTION_TYPE" in
  rollback|dr|manual_fix|change)
    ;;
  *)
    echo "ACTION_TYPE must be one of: rollback, dr, manual_fix, change" >&2
    exit 2
    ;;
esac
...
aws events put-events \
  --region "$AWS_REGION" \
  --entries "Source=cd.quality-gate,DetailType=DeploymentActionApproved,EventBusName=${EVENT_BUS_NAME},Detail=$(jq -c '.detail' "$OUTPUT_FILE")"
